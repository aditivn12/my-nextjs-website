...
export default function ProjectsInternshipsPage() {
  ...
  return (
    <div
      className={`min-h-screen bg-gradient-to-br from-[#ffe4d9] via-[#ffccf9] to-[#d5f4ff] text-zinc-800 px-6 py-16 transition-all duration-1000 ${
        visible ? "opacity-100 translate-y-0" : "opacity-0 translate-y-8"
      }`}
    >

      <div className="w-full flex flex-wrap justify-center gap-4 md:gap-6 pb-12">
        <Link
          href="/"
      ...
      <div className="max-w-5xl mx-auto space-y-20">
        ...
        <div className="text-center">
          ...
          <p className="text-zinc-600">
            A selection of full-stack tools and experiences I’ve built with care.
          </p>
        </div>
        ...
        <div className="grid md:grid-cols-2 gap-10">
          {projects.map((project, i) => (
            <div
              key={i}
              className={`bg-white border border-rose-100 rounded-3xl p-6 shadow-md hover:shadow-lg transition-all duration-700 ease-in-out hover:scale-[1.02] ${
                project.title.includes("Dog Adoption")
                  ? "md:col-span-2 md:w-full max-w-2xl mx-auto"
                  : ""
              }`}
            >
              <h2 className="text-xl font-bold text-rose-700 mb-3 text-center">
                {project.title}
              </h2>
        ...
        <div className="text-center pt-16">
          ...
          <p className="text-zinc-600">
            Highlights of collaborative work and hands-on learning.
          </p>
        </div>
        ...
        <div className="grid md:grid-cols-2 gap-6 lg:gap-8">
          {internships.map((intern, i) => (
            <div
              key={i}
              className={`bg-white border border-orange-100 rounded-3xl p-6 shadow-sm hover:shadow-md transition-all duration-700 ease-in-out hover:scale-[1.02] ${
                visible ? "opacity-100 translate-y-0" : "opacity-0 translate-y-8"
              } ${i === internships.length - 1 ? "md:col-span-2 md:w-full max-w-2xl mx-auto" : ""}`}
            >
              <h2 className="text-xl font-semibold text-orange-700 mb-2 text-center">
                {intern.title}
              </h2>
